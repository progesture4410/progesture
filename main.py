...
if "FIREBASE_KEY" in os.environ:
    ...
    # ✅ Local fallback (use your JSON file)
    ...
    if request.method == "POST":
        ...
        if updated:
            ...
            if sensitive_changed:
                # 🔥 ALWAYS GET LATEST USER REF (FIX)
                updated_doc_ref = db.collection("users").document(current_user).get()

                if not updated_doc_ref.exists:
                    flash("Error updating user. Please try again.")
                    return redirect(url_for("profile"))

                updated_doc = updated_doc_ref.to_dict()

                qr_folder = os.path.join("static", "qrcodes")
                os.makedirs(qr_folder, exist_ok=True)

                # 🔥 DELETE OLD QR IF USERNAME CHANGED
                old_qr_path = os.path.join(qr_folder, f"{username}.png")
                if os.path.exists(old_qr_path) and username != current_user:
